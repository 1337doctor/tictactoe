...
export const updateWinnerField = (row, col, val) => {
    document.getElementById(config.winnerFieldId).innerHTML= `Winner:  ${ gameState.winner ? getPlayerData(gameState.winner).icon : '-'}`;
}


export const updateTotalMovesField = () => {
    document.getElementById(config.movesLeftField).innerText = `Moves left:  ${possibleTotalMoves()}`;
}
...
export const wrapperPack = () => {
   const width = document.getElementById(config.tableId).getBoundingClientRect().width;
